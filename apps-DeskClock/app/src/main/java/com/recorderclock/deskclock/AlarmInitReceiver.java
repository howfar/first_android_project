...
public class AlarmInitReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        com.recorderclock.deskclock.AsyncHandler.post(new Runnable() {
            @Override
            public void run() {
                try {
                    // Process restored data if any exists
                    if (!com.recorderclock.deskclock.DeskClockBackupAgent.processRestoredData(context)) {
                    ...
                    result.finish();
                    wl.release();
                    com.recorderclock.deskclock.LogUtils.v("AlarmInitReceiver finished");
                }
            }
        });
    }
